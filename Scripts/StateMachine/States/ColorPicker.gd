...
export(float, 0.2, 1.0) var text_color_flip_threshold = 0.5
...
func _color():
	...
	if Engine.editor_hint and fresh:
		...
		canvas.rect_position = Vector2(20, 10)
...
func rect_changed():
	if center:
		center.rect_size = Vector2(200, 60)


func init_size():
	if Engine.editor_hint and fresh:
		canvas.rect_size = Vector2(200, 60)
		fresh = false
	rect_changed()
...
func show_panel():
	var movement_verfification = get_parent().get_parent().get_parent().get_parent().notyetmoved
	if movement_verfification:
		panel.popup_centered()
	else:
		
		var mouse = canvas.get_global_mouse_position()
